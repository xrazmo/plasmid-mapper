const ORF_COLOR = { "hypothetical": "#FEE08B", "transposase": "#225EA8", "isel": "#1D91C0", "integrase": "#41AB5D", "virulence": "#BF812D", "biocidemetal": "#FD8D3C", "unknown": "#969696", "other": "#99D8C9", "args": "#a50f15" };

const Mismatch_COLOR = { "m": "#f32707", "s": "#f32707", "q": "#f32707" }
    // const Color_collection = ["#FFC0CB", "#FF69B4", "#C71585", "#8B008B",
    //     "#6A5ACD",
    //     "#FFA07A", "#B22222", "#FF4500",
    //     "#ADFF2F", "#32CD32", "#228B22", "#556B2F",
    //     "#008B8B", "#4682B4", "#00BFFF", "#0000FF", "#000080",
    //     "#662506", "#cc4c02",
    //     "#081d58", "#225ea8",
    //     "#8c510a", "#bf812d",
    //     "#01665e", "#35978f",
    //     "#d73027", "#f46d43",
    //     "#66bd63", "#1a9850",
    //     "#B8860B", "#D2691E", "", "", "", "", "", "", "", "", ""
    // ]

const Color_collection = ["#003300", "#047c12", "#05ad19", " #20f83a", "#73e600", "#bfff80",
    "#002b80", "#003cb3", "#0055ff", "#0080ff", "#66b3ff",
    "#400080", " #7300e6", "#9933ff",
    "#89ecda", "#43e8d8", "#3bd6c6",
    "#a67c00", "#bf9b30", "#ffbf00", "#ffcf40", "#ffdc73",
    "#b2d8d8", "#66b2b2", "#008080", "#006666",
    "#eec1ad", "#dbac98", "#c98276",
    "#8f9779", "#78866b", "#4d5d53",
    "#fff2cc", "#f19ad3", "#66916d",
    "#464d6f", "#cee9e7",
    "#147a6f", "#e69138", "#b6d7a8"
]